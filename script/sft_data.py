...
支持生成训练集和测试集，确保 prompt 格式完全一致。

使用方法:
  # 简单模式 - 训练集
  python sft_data.py --src data/train.json --dst data/train_sft.json --split train
  
  # 简单模式 - 测试集
  python sft_data.py --src data/test.json --dst data/test_sft.json --split test
  
  # MRAG 模式 - 训练集
  python sft_data.py --src data/train.json --dst data/train_sft_mrag.json --split train \
      --use_mrag \
      --law_runfile data/retrieval/law_runfile_train \
      --law_corpus data/law_corpus.jsonl
  
  # MRAG 模式 - 测试集
  python sft_data.py --src data/test.json --dst data/test_sft_mrag.json --split test \
      --use_mrag \
      --law_runfile mrag/retriever_output/law_runfile_reranked_test.tsv \
      --law_corpus data/law_corpus.jsonl
"""
...
def build_case_id_mapping(case_corpus_path: str) -> Dict[str, tuple]:
    ...
    注意：使用 qw 字段作为相关案例判决书示例
    qw 是完整的判决书全文，fd 是案件事实部分
    """
    id_to_data = {}
...
def select_case_qw(case_qws: List[str]) -> Optional[str]:
    ...
    优先选择长度 <= MAX_CASE_LENGTH 的，否则选最短的
    """
    if not case_qws:
        return None
...
def process_data(
    ...
    output_path: Path,
    use_mrag: bool,
    law_results: Optional[Dict[str, List[str]]],
    case_results: Optional[Dict[str, List[str]]],
    split: str,
):
    """处理数据并写入文件
    ...
    with output_path.open("w", encoding="utf-8") as fout:
        for idx, item in enumerate(data):
            ...
            output = item.get("fd") or item.get("output") or ""
            text_id = str(item.get("text_id", idx))
            
            if not fact:
                continue
            
            # 测试集可能没有 output，但仍需生成
            if split == "train" and not output:
                continue
            
            # 构建 prompt
            if use_mrag and (law_results or case_results):
                laws = law_results.get(text_id, []) if law_results else []
                case_qw = None
                if case_results:
                    case_qws = case_results.get(text_id, [])
                    case_qw = select_case_qw(case_qws)
                
                input_prompt = build_mrag_prompt(fact, laws, case_qw)
            else:
                input_prompt = build_simple_prompt(fact)
...
def main():
    parser = argparse.ArgumentParser(description="Generate SFT training/test data")
    parser.add_argument("--src", type=Path, required=True, help="Source data file (train.json or test.json)")
    parser.add_argument("--dst", type=Path, required=True, help="Output file path")
    parser.add_argument("--split", type=str, choices=["train", "test"], default="train",
                        help="Data split type: train or test")
    
    # MRAG 参数
    parser.add_argument("--use_mrag", action="store_true", help="Enable MRAG mode")
    parser.add_argument("--law_runfile", type=Path, default=None, help="Law retrieval result file")
    parser.add_argument("--case_runfile", type=Path, default=None, help="Case retrieval result file")
    ...
    if args.use_mrag:
        if args.law_runfile and args.law_runfile.exists():
            ...
            print(f"Warning: Law runfile not found: {args.law_runfile}")
    ...
    # 处理数据
    process_data(data, args.dst, args.use_mrag, law_results, case_results, args.split)
