...
class RerankerTrainer(Trainer):
    ...
    def _get_train_sampler(self):
        ...
        if self.args.local_rank == -1:
            ...
            # local_rank >= 0 但分布式未初始化，使用 RandomSampler
            logger.warning("local_rank >= 0 but distributed not initialized, using RandomSampler")
            return RandomSampler(self.train_dataset)
        elif self.args.collaborative:
            # 分布式已初始化且使用 collaborative 模式
            logger.info(f'Collaborative Mode.')
            return SyncedSampler(self.train_dataset, seed=self.args.seed)
        else:
            # 分布式已初始化，使用 DistributedSampler
    ...
    def compute_loss(self, model: Reranker, inputs, **kwargs):
        """
        计算损失
        修复：接受额外的关键字参数（如 num_items_in_batch）以兼容新版本 Transformers
        """
